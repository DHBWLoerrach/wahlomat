...
function initOnclickCallbacks() {
  ...
  $('#btn-mahlowat-show-start')
    .off('click')
    .click(function () {
      showStart();
    });
  $('#btn-mahlowat-show-qa')
    .off('click')
    .click(function () {
      showQA();
    });
  $('#btn-mahlowat-skip-remaining-theses')
    .off('click')
    .click(function () {
      showResults();
    });
...
function initHammer() {
  var thesisHammer = new Hammer(
    document.getElementById('thesis-card')
  );
  thesisHammer
    .get('swipe')
  ...
  var resultHammer = new Hammer(
    document.getElementById('result-summary-row')
  );
...
function updateProgressBar() {
  let percentage = Math.round(
    (100 * (currentThesis + 1)) / Object.keys(data.theses).length
  );
...
function toggleImportant() {
  answers[currentThesis] = toggleImportantCharacter(
    answers[currentThesis]
  );
  if (isThesisMarkedImportant()) {
    setImportant();
  } else {
...
function showResults() {
  ...
  for (let i = 0; i < answers.length; i++) {
    maxAchievablePoints += calculatePairPoints(
      answers[i],
      answers[i]
    );
  }
  for (list_id in data.lists) {
    ...
    for (let i = 0; i < answers.length; i++) {
      let thesis_id = '' + i;
      pointsForList += calculatePairPoints(
        answers[i],
        data.answers[list_id][thesis_id].selection
      );
    }
...
function updateResultDetailPlaceholders() {
  for (let i = 0; i < answers.length; i++) {
    if (
      ...
      $('#placeholder-your-choice-' + i)
        .parent()
        .addClass('bg-info');
    $('#placeholder-your-choice-' + i).replaceWith(
      getSelectionMarker(t.label_your_choice, answers[i])
    );
  }
}
...
function addResultSummary(list, pointsForList, maxAchievablePoints) {
  let percentage = Math.round(
    (pointsForList / maxAchievablePoints) * 100
  );
  let remaining_percentage = 100 - percentage;
  let text_percentage = t.achieved_points_text(
    pointsForList,
    maxAchievablePoints
  );
...
function getSummaryProgressBar(
  list,
  percentage,
  remaining_percentage,
  text_percentage,
  ...
  let bar =
    '<div class="row result-summary-row">\
    ...
    for (list_id in data.lists) {
      text +=
        '<li class="list-group-item">\
							' +
        getSelectionMarker(
          data.lists[list_id].name,
          data.answers[list_id][thesis_id].selection
        ) +
        '\
							' +
        statementOrDefault(
          data.answers[list_id][thesis_id].statement
        ) +
        '</li>';
    }
    text +=
      '</ul>\
				...
				<div class="card-footer result-detail-footer">\
					<span class="badge badge-secondary" id="placeholder-your-choice-' +
      thesis_id +
      '">PLACEHOLDER</span> | ';
    for (list_id in data.lists) {
      text += getSelectionMarker(
        data.lists[list_id].name,
        data.answers[list_id][thesis_id].selection
      );
    }
